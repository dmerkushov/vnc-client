 ...
 /* Cursor control & motion */
	...
	public static int translateKeyEvent (KeyEvent ev) {
		...
		if (KeyEvent.getKeyText (keyCode).isEmpty ()) {
			...
			VncCommon.getLogger ().warning ("Unknown key code: " + String.format (fmt.replaceAll (",", "%n       ")));
			return VoidSymbol;
		}
		...
		if (ev.isControlDown ()) {
			...
			else if (ev.getKeyChar () == KeyEvent.CHAR_UNDEFINED && keyCode >= 0 && keyCode <= 127) {
				key = keyCode;
			}
		}
