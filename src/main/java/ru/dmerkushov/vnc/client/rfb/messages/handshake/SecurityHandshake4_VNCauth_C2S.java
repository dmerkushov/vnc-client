...
/**
 * This is the security handshake, phase 4, in case of VNC authentication. Sent by the client to the server (C2S) after
 * the user entered a password. Described in RFC 6143, paragraph 7.2.2
 *
 * @author dmerkushov
 */
public class SecurityHandshake4_VNCauth_C2S extends RfbMessage {
	...
	public SecurityHandshake4_VNCauth_C2S (RfbClientSession session, byte[] challenge, String password) throws MessageException {
		...
		if (password.length () > SecurityHandshake4_VNCauth_C2S.PASSWORD_STD_LENGTH) {
			password = password.substring (0, SecurityHandshake4_VNCauth_C2S.PASSWORD_STD_LENGTH);
		}

		byte[] passwordBytes;
		try {
			passwordBytes = Arrays.copyOf (password.getBytes (VncCommon.STRING_ENCODING), SecurityHandshake4_VNCauth_C2S.PASSWORD_STD_LENGTH);
		...
		desCipher.encrypt (challenge, 0, this.response, 0);
		desCipher.encrypt (challenge, 8, this.response, 8);
	}
	...
	public void write (OutputStream out) throws IOException {
		...
		out.write (Arrays.copyOfRange (this.response, 0, 16));
	}
	...
	public void read (InputStream in) throws IOException, MessageException {
		...
		this.response = readBytes (in, SecurityHandshake4_VNCauth_C2S.RESPONSE_STD_LENGTH);
	}
	...
	static class DesCipher {
		...
		private void deskey (byte[] keyBlock, boolean encrypting, int[] KnL) {
			...
			for (j = 0; j < 56; ++j) {
				l = DesCipher.pc1[j];
				m = l & 07;
				pc1m[j] = ((keyBlock[l >>> 3] & DesCipher.bytebit[m]) != 0) ? 1 : 0;
			}
			...
			for (i = 0; i < 16; ++i) {
				...
				for (j = 0; j < 24; ++j) {
					if (pcr[DesCipher.pc2[j]] != 0) {
						kn[m] |= DesCipher.bigbyte[j];
					}
					if (pcr[DesCipher.pc2[j + 24]] != 0) {
						kn[n] |= DesCipher.bigbyte[j];
					}
				}
			}
			this.cookey (kn, KnL);
		...
		public void encrypt (byte[] clearText, int clearOff, byte[] cipherText, int cipherOff) {
			DesCipher.squashBytesToInts (clearText, clearOff, this.tempInts, 0, 2);
			this.des (this.tempInts, this.tempInts, this.encryptKeys);
			DesCipher.spreadIntsToBytes (this.tempInts, 0, cipherText, cipherOff, 2);
		}

		/// Decrypt a block of eight bytes.
		public void decrypt (byte[] cipherText, int cipherOff, byte[] clearText, int clearOff) {
			DesCipher.squashBytesToInts (cipherText, cipherOff, this.tempInts, 0, 2);
			this.des (this.tempInts, this.tempInts, this.decryptKeys);
			DesCipher.spreadIntsToBytes (this.tempInts, 0, clearText, clearOff, 2);
		}
		...
		private void des (int[] inInts, int[] outInts, int[] keys) {
			...
			for (round = 0; round < 8; ++round) {
				...
				work ^= keys[keysi++];
				fval = DesCipher.SP7[work & 0x0000003f];
				fval |= DesCipher.SP5[(work >>> 8) & 0x0000003f];
				fval |= DesCipher.SP3[(work >>> 16) & 0x0000003f];
				fval |= DesCipher.SP1[(work >>> 24) & 0x0000003f];
				work = right ^ keys[keysi++];
				fval |= DesCipher.SP8[work & 0x0000003f];
				fval |= DesCipher.SP6[(work >>> 8) & 0x0000003f];
				fval |= DesCipher.SP4[(work >>> 16) & 0x0000003f];
				fval |= DesCipher.SP2[(work >>> 24) & 0x0000003f];
				leftt ^= fval;
